...
function FolderNode({ folder, allFolders, selectedFolderId, onSelect, disabledIds, depth = 0 }) {
  const children = allFolders.filter(f => f.parentId === folder.id && !disabledIds.includes(f.id));
  ...
  return (
    <div className="move-folder-node">
      <div
        className={`move-folder-row ${isSelected ? 'selected' : ''} ${isDisabled ? 'disabled' : ''}`}
        style={{ paddingLeft: `${12 + Math.min(depth, MAX_DEPTH) * 16}px` }}
        onClick={handleSelect}
      >
        <button
          className="move-chevron-btn"
          onClick={toggleOpen}
...
export default function MoveFolderModal({
  ...
  // Calculate disabled folders (the folder being moved and all its descendants)
  const disabledIds = useMemo(() => {
    const disabled = [folder.id];
    const addDescendants = (folderId) => {
      const children = folders.filter(f => f.parentId === folderId);
      children.forEach(child => {
        disabled.push(child.id);
  ...
  const rootFolders = useMemo(() =>
    folders.filter(f => f.parentId === null && !disabledIds.includes(f.id)),
    [folders, disabledIds]
  );
  ...
  const filteredFolders = useMemo(() => {
    ...
    return folders.filter(f =>
      f.name.toLowerCase().includes(query) && !disabledIds.includes(f.id)
    );
  }, [folders, searchQuery, disabledIds]);
